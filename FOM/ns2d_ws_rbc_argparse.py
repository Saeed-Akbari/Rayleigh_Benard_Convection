...
if ip == 1:
    ...
    directory = os.path.join(directory, f'solution_{nx}_{ny}_{ra:0.1e}_{its}_{isolver}_{icompact}')
    if not os.path.exists(directory):
        os.makedirs(directory)
...
#        print('%0.5i %0.3f %0.3e %0.3e %0.3e' % (kc[k], time[k], rw[k], rs[k], rth[k]), file=log)
    ...
    if current_time > t_movie:
        filename = os.path.join(directory_movie, f'{km}.npz')
        np.savez(filename,
                 w = w, s = s,th=th) 
    ...
    if k % sfreq == 0:
        ...
        filename = os.path.join(directory_save, f'{k}.npz')
        np.savez(filename,
                 w = w, s = s, th=th)  
